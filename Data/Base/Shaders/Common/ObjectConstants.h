...
#  include "Common.h"
StructuredBuffer<ezPerInstanceData> perInstanceData BIND_SET(SET_DRAW_CALL);

#  if defined(USE_SKINNING)
StructuredBuffer<Transform> skinningTransforms BIND_SET(SET_DRAW_CALL);
#  endif

#  if EZ_ENABLED(SUPPORTS_TEXEL_BUFFER)
Buffer<float4> perInstanceVertexColors BIND_SET(SET_DRAW_CALL);
#  else
StructuredBuffer<uint> perInstanceVertexColors BIND_SET(SET_DRAW_CALL);
#  endif
...
CONSTANT_BUFFER2(ezObjectConstants, 2, SET_DRAW_CALL)
{
